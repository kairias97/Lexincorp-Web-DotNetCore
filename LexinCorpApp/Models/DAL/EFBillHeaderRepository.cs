...
namespace LexincorpApp.Models
{
    public class EFBillRepository : IBillRepository
    {
        ...
        internal static readonly Dictionary<BillDetailTypeEnum, int> detailOrder = new Dictionary<BillDetailTypeEnum, int>()
        {
            {BillDetailTypeEnum.Package, 1 },
            {BillDetailTypeEnum.PackageDetail, 2 },
            {BillDetailTypeEnum.Item, 3 },
            {BillDetailTypeEnum.Retainer, 4 },
            {BillDetailTypeEnum.RetainerDetail, 5 },
            {BillDetailTypeEnum.Hours, 6 },
        ...
        public BillHeader GenerateBill(BillRequest billRequest, int userId, out bool IsEnglishBill)
        {
            ...
            {
                decimal timeWorked = context.Activities
                    .Where(a => a.PackageId == p.Id)
                    .Sum(a => a.HoursWorked);
            ...
            var itemDetail = hourlyOrItemDetails
                .Where(d=> d.BillDetailType == BillDetailTypeEnum.Item)
                .OrderBy(d => d.Date)
                .ToList();
            var itemSummary =hourlyOrItemSummary
                .Where(s => s.TypeId == BillDetailTypeEnum.Item)
                .OrderBy(s => s.Date)
                .ToList();

            if (itemDetail.Count > 0)
            {
                details.AddRange(itemDetail);
            }
            if (itemSummary.Count > 0)
            {
                summary.AddRange(itemSummary);
            }
            ...
            var hourDetail = hourlyOrItemDetails
                .Where(d => d.BillDetailType == BillDetailTypeEnum.Hours)
                .OrderBy(d => d.Date)
                .ToList();
            var hourSummary = hourlyOrItemSummary
                .Where(s => s.TypeId == BillDetailTypeEnum.Hours)
                .OrderBy(s => s.Date)
                .ToList();

            if (hourDetail.Count > 0)
            {
                details.AddRange(hourDetail);
            }
            if (hourDetail.Count > 0)
            {
                summary.AddRange(hourSummary);
            }



            //Logic for adding the rest of expenses for the rest of activities
            List<int> otherIds = details
                .Where(d => d.ActivityId != null && d.BillDetailType != BillDetailTypeEnum.Package && d.BillDetailType != BillDetailTypeEnum.PackageDetail && d.BillDetailType != BillDetailTypeEnum.Retainer)
                .Select(d => (int)d.ActivityId)
                .ToList();
            var otherExpenses = context.ActivityExpenses
                .Include(ae => ae.Expense)
                .Where(ae => otherIds.Contains(ae.ActivityId))
                .ToList();

            foreach (var e in otherExpenses)
            {
                var expenseToAdd = new BillExpense
                {
                    Date = e.RealizationDate,
            ...
            //Finishing up getting the rest of expenses
            billHeader.Active = true;
            totalExpenses = expenses.Sum(e => e.Subtotal);


            billHeader.BillSummaries = summary;
            ...
            var activities = billHeader.BillDetails
                .Where(d => d.BillDetailType != BillDetailTypeEnum.Package && d.BillDetailType != BillDetailTypeEnum.Retainer)
        ...
        //}
        //}
        public void DisableBill(int billId)
        {
            var bill = context.BillHeaders
                .Include(b => b.BillDetails)
                .Where(b => b.Id == billId)
                .First();
            bill.Active = false;
            var activitiesToRollback = bill.BillDetails
                ...
                .ToList();
            var packagesToRollback = bill.BillDetails
            ...
            var retainersToRollback = bill.BillDetails
                .Where(d => d.BillableRetainerId != null)
                .Select(d => new BillableRetainer
                {
                    Id = (int)d.BillableRetainerId,
                    IsBilled = false,
                    IsBillable = false
                }).ToList();
            if (activitiesToRollback.Any())
            {
                context.Activities.AttachRange(activitiesToRollback);
                foreach (var a in activitiesToRollback)
                {
                    context.Entry(a).Property(act => act.IsBillable).IsModified = true;
                    context.Entry(a).Property(act => act.IsBilled).IsModified = true;
                }
            }
            if (packagesToRollback.Any())
            {
                context.Packages.AttachRange(packagesToRollback);
                foreach (var p in packagesToRollback)
                {
                    context.Entry(p).Property(a => a.IsBilled).IsModified = true;
                }
            }
            if (retainersToRollback.Any())
            {
                context.BillableRetainers.AttachRange(retainersToRollback);
                foreach (var r in retainersToRollback)
                {
                    context.Entry(r).Property(a => a.IsBillable).IsModified = true;
                    context.Entry(r).Property(a => a.IsBilled).IsModified = true;
                }
            }
            context.SaveChanges();
        }
